...
namespace IMAGE_FILTERS
{
    public class ImageOperations
    {
        ...
        public static byte Filter2(byte[,] ImageMatrix, int x, int y, int T, int Wmax, int Sort)
        {
            byte[] Array;
            int[] Dx, Dy;
            if (Wmax % 2 != 0)
        ...
        ////
        public static byte Filter1(byte[,] ImageMatrix, int x, int y, int W, int Wmax, int Sort)
        {
            ...
            if (A1 > 0 && A2 > 0)
            {
                ...
                {
                    if (W + 2 < Wmax)
                        return Filter1(ImageMatrix, x, y, W + 2, Wmax, Sort);
                    else
                        return Med;
                }
            }
        ...
        ////
        public static byte[,] Adaptive_median(byte[,] ImageMatrix, int Max_Size, int Sort)
        {
            ...
            for (int y = 0; y < GetHeight(ImageMatrix); y++)
            {
                ...
                {
                    ImageMatrix2[y, x] = Filter1(ImageMatrix, x, y,5,Max_Size, Sort);
                }
            }

            return ImageMatrix2;
        }
        public static byte[,] Alpha_trim(byte[,] ImageMatrix, int Max_Size, int Sort, int T)
        {
            byte[,] ImageMatrix2 = ImageMatrix;
            for (int y = 0; y < GetHeight(ImageMatrix); y++)
            {
                for (int x = 0; x < GetWidth(ImageMatrix); x++)
                {
                        ImageMatrix2[y, x] = Filter2(ImageMatrix, x, y,T, Max_Size, Sort);
                }
            }
        ...
        public static double CalculateTime(byte[,] ImageMatrix, int Max_Size, int filter)
        {
            PictureBox Picture_Filter = new PictureBox();
            int Start = System.Environment.TickCount;
            if (filter == 0)
                ImageOperations.Adaptive_median(ImageMatrix, Max_Size, 1);
            else
                ImageOperations.Alpha_trim(ImageMatrix, Max_Size, 1, 3);
